...
//    protected $need_mysql = true;
    ...
    public function returnWebAppUrl($data, $lang)
    {
        $lang = strtolower($lang);
        $inline_keyboard = new InlineKeyboard(
            [['text' => Yii::t('app', 'Osago', [], $lang), 'web_app' => ['url' => GeneralHelper::env('front_website_url') . "/" . $lang . '/osago']]],
            [['text' => Yii::t('app', 'Kasko by subscription', [], $lang), 'web_app' => ['url' => GeneralHelper::env('front_website_url') . "/" . $lang . '/kasko-subscription']]],
            [['text' => Yii::t('app', 'Kasko', [], $lang), 'web_app' => ['url' => GeneralHelper::env('front_website_url') . "/" . $lang . '/kasko/calculator']]]
        );

        $data['text'] = Yii::t('app', "Qaysi produkt bo`yicha polis rasmiylashtirmoqchisiz?", [], $lang);
//        $data['reply_markup'] = Keyboard::remove(['selective' => true]);
    ...
    public function execute(): ServerResponse
    {
        ...
        $token = Token::findOne(['telegram_chat_id' => $chat_id, 'status' => Token::STATUS['verified']]);
        if (!is_null($token))
            $user = User::findOne($token->f_user_id);
        ...
        if (!is_null($user) and !empty($token->telegram_lang))
        {
            $notes['state'] = 3;
            $notes['lang'] = $token->telegram_lang;
        }
        ...
        switch ($state) {
            ...
            case 3:
                $write_to_chat_text = Yii::t('app', "write_to_chat", [], $notes['lang']);
                $buy_policy_text = Yii::t('app', "buy_policy", [], $notes['lang']);

                if ($text === '' || !in_array($text, [$write_to_chat_text, $buy_policy_text], true)) {
                    $notes['state'] = 3;
                    $this->conversation->update();

                    $data['reply_markup'] = (new Keyboard(
                        (new KeyboardButton($write_to_chat_text)),
                        (new KeyboardButton($buy_policy_text))
                    ))
                        ->setOneTimeKeyboard(false)
                        ->setResizeKeyboard(true)
                        ->setSelective(false);
                    $data['text'] = Yii::t('app', "Tabriklaymiz, siz muvaffaqiyatli ro'yxatdan o'tdingiz!", [], $notes['lang']);
                    if (!is_null($text)) {
                        $data['text'] = Yii::t('app', 'Iltimos quyidagi tugmalardan tanlang', [], 'uz');
                    }
                ...
                if ($text == $buy_policy_text)
                    $result = $this->returnWebAppUrl($data, $notes['lang']);
                elseif ($text == $write_to_chat_text)
                {
                    $lang = strtolower($notes['lang']);
                    $inline_keyboard = new InlineKeyboard([
                        ['text' => Yii::t('app', 'chat', [], $lang), 'url' => "https://t.me/Sugurtabozorsupport_bot"]
                    ]);

                    $data['text'] = Yii::t('app', "Siz quyidagi havola orqali o'tib bizning operatorimizga savol berishingiz mumkin", [], $lang);
                    $data['reply_markup'] = $inline_keyboard;
                    $result = Request::sendMessage($data);
                }
