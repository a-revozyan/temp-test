...
class SendPolicyExpireWarningMessageController extends Controller
{
    public function actionRun()
    {
        ...
        $day3_osagos = (clone $osagos_query)->where(['end_date' => date('Y-m-d', strtotime("+3 days"))])->asArray()->all();
        $day2_osagos = (clone $osagos_query)->where(['end_date' => date('Y-m-d', strtotime("+2 days"))])->asArray()->all();
        $day1_osagos = (clone $osagos_query)->where(['end_date' => date('Y-m-d', strtotime("+1 days"))])->asArray()->all();
        ...
        foreach ($day2_osagos as $osago) {
            ['url' => $short_url, 'code' => $code] = $this->getShortLink($osago['id']);
            Yii::$app->queue1->push(new SendWarningSmsJob([
                'day' => 2,
                'code' => $code,
                'phone' => $osago['phone'],
                'message' => "Sug'urta Bozor. Polis OSAGO dlya ". $osago['autonumber'] ." istekaet cherez 2 dnya. Kupit' polis: $short_url",
            ]));
        }

        foreach ($day1_osagos as $osago) {
            ['url' => $short_url, 'code' => $code] = $this->getShortLink($osago['id']);
            Yii::$app->queue1->push(new SendWarningSmsJob([
                'day' => 1,
                'code' => $code,
                'phone' => $osago['phone'],
                'message' => "Sug'urta Bozor. Srochno. Polis OSAGO dlya ". $osago['autonumber'] ." istekaet. Obnovit' polis: $short_url",
            ]));
        }
...
//        $this->sendStrangerOsagoPolicy();
    ...
    public function sendOldOsago()
    {
        ...
        $day2_osagos = (clone $osagos_query)->andWhere([
            'or',
            [
                'and',
                ['=', new Expression('created_at::timestamp::date'), date('Y-m-d', strtotime("-6 months, +2 days"))],
                ['in', 'amount_uzs', [28000, 84000, 117600]]
            ],
            [
                'and',
                ['=', new Expression('created_at::timestamp::date'), date('Y-m-d', strtotime("-12 months, +2 days"))],
                ['in', 'amount_uzs', [40000, 120000, 168000]]
            ]
        ])->asArray()->all();
        $day1_osagos = (clone $osagos_query)->andWhere([
            'or',
            [
                'and',
                ['=', new Expression('created_at::timestamp::date'), date('Y-m-d', strtotime("-6 months, +1 days"))],
                ['in', 'amount_uzs', [28000, 84000, 117600]]
            ],
            [
                'and',
                ['=', new Expression('created_at::timestamp::date'), date('Y-m-d', strtotime("-12 months, +1 days"))],
                ['in', 'amount_uzs', [40000, 120000, 168000]]
            ]
        ])->asArray()->all();

//        $long_url = "https://sugurtabozor.uz/osago";
//        $short_url = ShortLink::findOrCreate($long_url)->short_url;
        ...
        foreach ($day2_osagos as $osago) {
            Yii::$app->queue1->push(new SendMessageJob([
                'phone' => $osago['insurer_phone_number'],
                'message' => "Sug'urta Bozor. Polis OSAGO ". $osago['policy_number'] ." istekaet cherez 2 dnya. Kupit' polis: $short_url",
            ]));
        }
        ...
        foreach ($day1_osagos as $osago) {
            Yii::$app->queue1->push(new SendMessageJob([
                'phone' => $osago['insurer_phone_number'],
                'message' => "Sug'urta Bozor. Srochno. Polis OSAGO ". $osago['policy_number'] ." istekaet. Obnovit' polis: $short_url",
            ]));
        }
    }
...
//        $short_url = ShortLink::findOrCreate($long_url)->short_url;
        ...
        $day2_osagos = (clone $stranger_osagos)->andWhere(['end_date' => date('Y-m-d', strtotime("+2 days"))])->asArray()->all();
        $day1_osagos = (clone $stranger_osagos)->andWhere(['end_date' => date('Y-m-d', strtotime("+1 days"))])->asArray()->all();
        ...
        foreach ($day2_osagos as $osago) {
            Yii::$app->queue1->push(new SendMessageJob([
                'phone' => $osago['phone_number'],
                'message' => "Sug'urta Bozor. Polis OSAGO na mashinu " . $osago['autonumber'] . " istekaet cherez 2 dnya. Kupit' polis: $short_url",
            ]));
        }
        ...
        foreach ($day1_osagos as $osago) {
            Yii::$app->queue1->push(new SendMessageJob([
                'phone' => $osago['phone_number'],
                'message' => "Sug'urta Bozor. Polis OSAGO na mashinu " . $osago['autonumber'] . " istekaet cherez 1 dnya. Kupit' polis: $short_url",
            ]));
        }
    }
    ...
    public function sendStrangerKaskoPolicy()
    {
        ...
        $day2_kaskos = (clone $stranger_kaskos)->andWhere(['end_date' => date('Y-m-d', strtotime("+2 days"))])->asArray()->all();
        $day1_kaskos = (clone $stranger_kaskos)->andWhere(['end_date' => date('Y-m-d', strtotime("+1 days"))])->asArray()->all();
        ...
        foreach ($day2_kaskos as $kasko) {
            Yii::$app->queue1->push(new SendMessageJob([
                'phone' => $kasko['phone_number'],
                'message' => "Sug'urta Bozor. Polis KASKO na mashinu " . $kasko['autonumber'] . " istekaet cherez 2 dnya. Kupit' polis: $short_url",
            ]));
        }

        foreach ($day1_kaskos as $kasko) {
            Yii::$app->queue1->push(new SendMessageJob([
                'phone' => $kasko['phone_number'],
                'message' => "Sug'urta Bozor. Polis KASKO na mashinu " . $kasko['autonumber'] . " istekaet cherez 1 dnya. Kupit' polis: $short_url",
            ]));
        }
    }
}