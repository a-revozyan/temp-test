...
class ImportAndSyncWithGrossAccidentForm extends Model
{
    public $excel_file;
...
//            Osago::updateAll(['gross_auto_id' => $gross_auto->id], ['policy_number' => $csv[4]]);
        ...
        $our_osago_arr = Accident::find()->select(['id', 'policy_number'])
            ->andWhere(['status' => Accident::STATUS['received_policy']])
        ...
        $diff_osagos = Accident::find()->select(['id', 'status', 'policy_number', 'policy_pdf_url'])->where(['in', 'policy_number', $diff_policies])->asArray()->all();
