...
class UpdateStoryForm extends Model
{
    public $name;
    public $type;
    public $files;
    ...
    {
        return [
            [['name', 'type', 'status', 'id'], 'required'],
            [['name'], 'string'],
            [['name'], 'filter', 'filter' => 'trim'],
            [['type', 'priority', 'status', 'view_condition', 'id'], 'integer'],
            ...
            [['files'], 'max5'],
            [['cover_file'], 'file', 'skipOnEmpty' => true],
            ...
            [['id'], 'exist', 'skipOnError' => true, 'targetClass' => Story::className(), 'targetAttribute' => ['id' => 'id']],
        ];
    }
    ...
    public function save()
    {
        ...
        {
            $story->coverFile[0]->deleteWithFile();
            $this->saveFile(StoryFile::TYPE['cover'], $this->cover_file, $story);
        }

        return Story::findOne($this->id);
    }
